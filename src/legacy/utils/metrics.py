...
class metricor:
    ...
    def new_sequence(self, label, sequence_original, window):
        a = max(sequence_original[0][0] - window, 0)
        sequence_new = []
        for i in range(len(sequence_original) - 1):
            if sequence_original[i][1] + window < sequence_original[i + 1][0] - window:
                sequence_new.append((a, sequence_original[i][1] + window))
                a = sequence_original[i + 1][0] - window
        sequence_new.append((a, min(sequence_original[len(sequence_original) - 1][1] + window, len(label) - 1)))
        return sequence_new
    ...
    def sequencing(self, x, L, window=5):
        ...
        zita = 1/np.sqrt(2)
        pos_slope = np.linspace(zita, 1, window+1)
        neg_slope = pos_slope[::-1]
        for k in range(len(L)):
            s = L[k][0]
            e = L[k][1]

            # x1 = np.arange(e + 1, min(e + window + 1, length))
            # label[x1] = np.maximum(label[x1], np.sqrt(1 - (x1 - e) / (2*window)))
            x1 = np.arange(e, min(e + window + 1, length))
            label[x1] = np.maximum(label[x1], neg_slope[:len(x1)])
        
            # x2 = np.arange(max(s - window, 0), s)
            # label[x2] = np.maximum(label[x2], np.sqrt(1 - (s - x2) / (2*window)))
            x2 = np.arange(max(s - window, 0), s + 1)
            label[x2] = np.maximum(label[x2], pos_slope[-len(x2):])
    ...
    def RangeAUC_volume_opt(self, labels_original, score, windowSize, thre=250):
        ...
        P = np.sum(labels_original)											# total numer of positives (in the label)
        sequences = self.range_convers_new(labels_original)					# Initial anomaly sequences, before enlarging them with slopes
        l = self.new_sequence(labels_original, sequences, windowSize)		# Non-static segments, aka max slope
        
        thresholds = np.sort(np.unique(score))[::-1]
        thre = thresholds.shape[0]
        ...
        for window in window_3d:	# Loop over the different slopes' lengths, window is int
            ...
            for i, threshold in enumerate(thresholds):
                ...
                for seg in L:	# loop over current slope anomalies
                    labels[seg[0]:seg[1] + 1] = labels_extended[seg[0]:seg[1] + 1] * pred[seg[0]:seg[1] + 1]	# multiply sloped-labels with predictions to 'turn off' unused slopes 
                ...
                TP = 0
                N_labels = 0
                for seg in l:	# non-static segments, max slope
                    TP += np.dot(labels[seg[0]:seg[1] + 1], pred[seg[0]:seg[1] + 1])	# Adding to all_TP the current slope's TPs, MULTIPLIED TWICE 
                ...
                N_new = len(labels) - P_new		# total negatives
                FPR = FP / N_new
            ...
            fpr_3d[window] = TF_list[:, 1]		# List of FPRs for every threshold, for the current window
            prec_3d[window] = Precision_list	# List of precisions for every threshold, for the current window
    ...
    def RangeAUC_volume_opt_mem(self, labels_original, score, windowSize, thre=250, existence_flag=True):
        ...
        for window in window_3d:
            ...
            for i in range(len(thresholds)):
                ...
                # curr_tp.append(TP)
                # curr_fp.append(FP)
                # curr_existence.append(existence)
            ...
            # tp.append(curr_tp)
            # fp.append(curr_fp)
            # existence_list.append(curr_existence)
        
        # For checking TP, FP, existence
        # tp = np.array(tp)
        # fp = np.array(fp)
        # existence_list = np.array(existence_list)
        # print(fp)
        # print(fp)
        ...
        # return tpr_3d, fpr_3d, prec_3d, window_3d, sum(auc_3d) / len(window_3d), sum(ap_3d) / len(window_3d)
        return tpr_3d, fpr_3d, prec_3d, window_3d, sum(ap_3d) / len(window_3d)    

    def sylli_RangeAUC_volume_opt_mem(self, labels_original, score, windowSize, thre=250):
        window_3d = np.arange(0, windowSize + 1, 1)
        P = np.sum(labels_original)
        seq = self.range_convers_new(labels_original)
        l = self.new_sequence(labels_original, seq, windowSize)
        ...
        for window in window_3d:
            ...
            AP_range = np.dot(width_PR, height_PR)
            ap_3d[window] = (AP_range)
        return sum(ap_3d) / len(window_3d)
    
    def sylli_RangeAUC(self, labels, score, window):
        thresholds = np.sort(np.unique(score))[::-1]
        P = np.sum(labels)
        labels = self.extend_postive_range(labels, window=window)
        
        L = self.range_convers_new(labels)
        TF_list = np.zeros((len(thresholds) + 2, 2))
        Precision_list = np.ones(len(thresholds) + 1)
        j = 0
